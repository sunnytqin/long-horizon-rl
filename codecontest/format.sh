#!/usr/bin/env bash
# Format codecontest/ in Google style, matching what gpylint expects in google3.
#
# codecontest/ is deliberately scoped out of verl's ruff config (see pyproject.toml
...
# Every .py under codecontest/, including tests/ -- `codecontest/*.py` would silently
# cover only the 12 top-level files out of 16.
mapfile -t files < <(find codecontest -name '*.py' -not -path '*__pycache__*')
