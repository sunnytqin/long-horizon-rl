...
class BaseEnv:
  ...
  def step(self, assistant_text: str) -> StepResult:
    """Grades one assistant submission and says what happens next.

    Args:
      assistant_text: the raw assistant turn, from which a ```python block is
        extracted.

    Returns:
      The ``StepResult`` for this turn.

    Raises:
      NotImplementedError: always; subclasses must provide the grading policy.
    """
...
class GTOracleEnv(BaseEnv):
  ...
  Attributes:
    test_input: ground-truth stdin cases (the same set used for feedback and
      final reward, per the eval protocol).
    test_output: the expected stdout for each entry of ``test_input``.
    ...
    _rng: the seeded RNG itself, built in ``__post_init__``.
  """
  ...
  def step(self, assistant_text: str) -> StepResult:
    """Grades the latest submission against the ground-truth tests.

    Args:
      assistant_text: the raw assistant turn to extract code from.

    Returns:
      A ``StepResult``: solved/terminate flags plus the failing-case feedback
      to inject as the next user turn when not terminating.
    """
    ...
    all_pass, _, failures = exec_client.eval_code_on_tests(
        code,
        self.test_input,
        self.test_output,
...
class TesterPolicyEnv(BaseEnv):
  ...
  Attributes:
    test_input: ground-truth stdin cases, if any are available to seed the
      tester with. Unused until ``step`` is implemented.
    test_output: the expected stdout for each entry of ``test_input``.
  """
  ...
  def step(self, assistant_text: str) -> StepResult:  # pragma: no cover - stub
    ...
    Args:
      assistant_text: the raw assistant turn that would be graded.

    Returns:
      Never returns.

    Raises:
      NotImplementedError: always, until tester+solver co-training lands.
    """
    raise NotImplementedError(
