...
inference hyper-parameters (temperature, top_p, number of turns, per-turn token
budget, etc.). The point is to manually examine what a trained checkpoint
actually does, so every trajectory is dumped in human-readable "conversation"
form to a JSON file.

``--feedback_mode`` selects which training loop to mirror, so a checkpoint is
always evaluated in the regime it was trained in (apples-to-apples):

  oracle
    Raw failing cases injected as the next user turn
    (``code_refine_agent.CodeRefineAgentLoop``).

  model_feedback
    A SECOND policy call (the SAME model, as a "user model") reads the failing
    cases and writes a diagnosis; only that diagnosis is injected and the raw
    cases are hidden (``model_feedback_agent.ModelFeedbackAgentLoop``).

The between-turns feedback is the ONLY thing that differs; grading and reward
are identical. Both modes share the number-affecting transforms with training
by importing the same ``codecontest.templates`` helpers (feedback formatting,
``normalize_diagnosis``).

Uses SGLang's offline ``Engine`` (same inference backend as the training
rollout), so it runs in the very same SGLang container as training -- no vLLM
dependency.

The heavy lifting is reused verbatim from the training path:
  - ``codecontest.env.GTOracleEnv`` -- grade latest code vs GT tests, build
    feedback
...
The conversation flow mirrors ``CodeRefineAgentLoop.run`` exactly:
  ...
  turn 1..N-1 : env grades the code; on failure injects failing cases as a
                user turn and the solver refines
  termination : all GT tests pass (early stop), max assistant turns reached,
                or the cumulative response-token budget would overflow.
Final reward is binary: 1.0 iff the final code passes all GT tests, else 0.0.

The one intentional difference vs the RL loop: we re-render the full message
list with the chat template each turn (clean, inspectable conversations) instead
of appending raw token ids. The model sees an equivalent prompt; there are no
...
Grading backend (identical semantics to training):
  ...
  - In-process fallback (dev/smoke, no sidecar):
...
Sweep several temperatures in ONE submission (engine loaded once). Each
temperature writes its own JSON with the config tagged into the name (turns /
n_samples / temp), so --out runs/eval_step120.json with --max_assistant_turns 4
--n_samples 8 --temperatures 0.0 0.8 produces
runs/eval_step120_turns4_n1_t0.json and runs/eval_step120_turns4_n8_t0.8.json
...
# growing conversation, its GT oracle env, and the running token-budget
# bookkeeping.
# --------------------------------------------------------------------------- #
class Trajectory:
  ...
  def __init__(
    ...
    self.response_budget = (
        response_budget  # max cumulative response tokens (assistant+feedback)
    )
    # Initial problem-statement user turn, reused as the `problem` field of the
    # user-model feedback prompt in --feedback_mode model_feedback (mirrors the
    # agent loop, which grabs the last user turn from raw_prompt). Unused in
    # oracle mode.
    ...
    self.turn_records = []
    # Model-feedback bookkeeping (mirrors ModelFeedbackAgentLoop's
    # extra_fields). Empty/unused in oracle mode.
    self.feedback_turn_lengths = (
        []
    )  # user-model diagnosis lengths (tokens), per feedback turn
    self.feedback_empty = (
        0  # diagnoses that came back empty (fell back to the fixed string)
    )
...
def build_trajectories(val_df, n_samples, eval_args):
  ...
  for row_index, row in val_df.iterrows():
    ...
    # Problem text for the user-model feedback prompt (model mode): the last
    # user turn, which wraps the problem statement -- exactly what the agent
    # loop uses.
    problem_text = next(
    ...
    for s in range(n_samples):
      env = GTOracleEnv(
          ...
          # Match the training loop's per-sample seed (env uses it to sample
          # which failing cases to show). Vary by sample so reruns are
          # reproducible.
...
def pass_at_k(n, c, k):
  """Unbiased pass@k estimator (Kulal et al. / HumanEval).

  The probability that at least one of k samples drawn WITHOUT replacement from
  n total (c of which succeed) is a success.
...
def per_turn_pass_at_k(by_problem, max_turns):
  ...
  For each turn cutoff ``t`` (0..max_turns-1) a sample counts as a success iff
  it solved *by* turn t, i.e. ``traj.solved and traj.solved_at_turn <= t``. NOTE
  we deliberately use ``solved_at_turn`` rather than ``turn_records[t].solved``
  because an early-solving trajectory stops and has no records past its solve
  turn -- the cutoff form is monotonic and correct. Each problem contributes its
  own (n, c_t); we average the unbiased pass@k over problems, reporting the full
  k=1..n curve (per-problem n; smaller groups skip larger k).
...
def eval_tagged_path(base_out, turns, n_samples, temperature, feedback_mode):
  """Inserts a '_turns<N>_n<K>_t<temp>_<mode>' tag before the extension.

  Each eval config therefore gets its own self-documenting file (and can never
  clobber a different config).

  ``n_samples`` is the temperature-adjusted value actually used (t=0 is forced
  to 1), so the tag faithfully records what was run. ``feedback_mode``
  (oracle|model_feedback) is tagged too (as oracle|modelfb) so an oracle eval
  and a model-feedback eval of the SAME checkpoint never collide.
  ...
  return (
      f"{root}_turns{turns}_n{n_samples}_t{temperature:g}_{mode_tag}"
      f"{ext or '.json'}"
  )


def build_next_user_turns(
    pending, feedback_mode, llm, tokenizer, args, max_model_len, sampling_params
):
  ...
  - oracle mode: the env already built the failing-case feedback -> inject
    ``step.feedback``.
  - model mode : a SECOND policy call (the "user model") writes the diagnosis,
    exactly like ``ModelFeedbackAgentLoop``. Trajectories that surfaced failing
    cases are batched into a single ``llm.generate`` (throughput), their
    diagnoses normalized by the SHARED ``templates.normalize_diagnosis``
    (identical <think>-strip + empty fallback as training) and wrapped by
    ``templates.build_model_feedback_user_message``. Trajectories with no
  ...
  # prompt_length + response_length, the SGLang context window.
  engine_ctx = max_model_len
  # Small margin so input+completion stays STRICTLY under engine_ctx: SGLang
  ...
  for i, (t, step) in enumerate(pending):
    ...
    # Throwaway feedback prompt: rendered UNCAPPED (no solver prompt_length
    # truncation, which would drop the problem), bounded only by the engine
    # context below.
    fb_prompt_text = tokenizer.apply_chat_template(
        fb_messages, add_generation_prompt=True, tokenize=False
    )
    fb_input_len = len(
        tokenizer.encode(fb_prompt_text, add_special_tokens=False)
    )
    # Cap the diagnosis (mirrors the agent loop): <= max_feedback_tokens, <=
    # remaining response budget (it lands in the response tail), and never
    # overflow the engine ctx.
    ...
    if fb_cap < 1:
      ...
      # the user-model call and inject the generic fallback -- keeps
      # model-feedback semantics (no raw cases leaked); the later overflow check
      # still handles the response budget.
      contents[i] = templates.build_model_feedback_user_message(
          templates.EMPTY_DIAGNOSIS_FALLBACK
      )
      t.feedback_empty += 1
      continue
...
def run_eval(
  ...
  """Runs the multi-turn oracle eval at one temperature, dumping one JSON.
  ...
  t0 = time.time()
  # ---- multi-turn loop ----
  # Every turn processes ALL still-active trajectories as one SGLang batch,
  # then grades the batch in parallel.
  for turn in range(args.max_assistant_turns):
    ...
    # the model's context window. The cumulative-response check above only
    # bounds response_tokens; it does NOT bound (initial_prompt + accumulated
    # turns + max_new_tokens_per_turn) against max_model_len, so on later turns
    # a fixed max_new_tokens request can exceed context and make SGLang raise
    # ValueError, killing the whole run. Guard per-request instead: shrink the
    # completion, or (if there's no room to generate at all) stop the trajectory
    # as overflow.
    ...
    clamped = sum(
        1
        for sp in per_req_params
        if sp["max_new_tokens"] < args.max_new_tokens_per_turn
    )
    print(
        f"[validate] t={temperature:g} turn {turn}: generating for "
        f"{len(kept)} active trajectories"
    ...
    # First pass: record each grade and classify. Trajectories that failed but
    # can still refine are collected for feedback (built in one batch below).
    ...
    # feedback; model runs a batched "user model" diagnosis call (see the
    # helper).
    ...
    # Second pass: inject each user turn, charging its tokens to the response
    # budget (matching the agent loop's accounting; overflow => stop unsolved).
  ...
  pass_at_1 = sum(1 for grp in by_problem.values() if grp[0].solved) / max(
      1, n_problems
  )
  # "final" pass@n: fraction of problems solved by ANY of the n samples at
  # end-of-conversation (naive any-solved rate, kept for backward-compat; the
  # unbiased curve lives in per_turn).
  pass_at_n = sum(
      1 for grp in by_problem.values() if any(t.solved for t in grp)
  ) / max(1, n_problems)
  solved_trajs = [t for t in trajs if t.solved]
  avg_turns_to_solve = (
      sum(t.solved_at_turn + 1 for t in solved_trajs) / len(solved_trajs)
      if solved_trajs
      else None
  )
  overflow_rate = sum(1 for t in trajs if t.overflow) / max(1, n_traj)
  # Per-turn pass@k surface: for each turn cutoff, the sample-averaged unbiased
  # pass@k over problems. per_turn[0]["pass@k"]["1"] is the single-turn (turn-0)
  # solve rate that should match a matched-n single-turn run's pass@1.
  ...
  # Model-feedback diagnostics (user-model diagnosis length + empty-diagnosis
  # rate), mirroring ModelFeedbackAgentLoop's feedback_resp_len_mean /
  # feedback_empty. Only meaningful in model mode; omitted in oracle mode where
  ...
  if args.feedback_mode == "model_feedback":
    ...
    fb_means = [
        ...
        for t in trajs
        if t.feedback_turn_lengths
    ]
    summary["feedback_resp_len_mean"] = (
        (sum(fb_means) / len(fb_means)) if fb_means else 0.0
    )
  ...
  with open(out_path, "w") as f:
    json.dump(
        {"summary": summary, "trajectories": [t.to_dict() for t in trajs]},
        f,
        indent=2,
    )
...
def main():
  ...
  ap.add_argument(
      "--model",
      required=True,
      help="HF model dir or name (a merged checkpoint).",
  )
  ap.add_argument(
      "--val_file",
      default=os.path.expanduser("~/data/codecontests/test.parquet"),
      help=(
          "Validation/test parquet (VERL schema from "
          "preprocess_codecontests.py)."
      ),
  )
  ap.add_argument(
      "--out", required=True, help="Output JSON path for the conversation dump."
  )
  ap.add_argument(
      "--max_problems",
      type=int,
      default=None,
      help="Limit #problems (debug). None = all.",
  )
  ap.add_argument(
      "--n_samples",
      type=int,
      default=1,
      help="Trajectories sampled per problem (pass@k).",
  )
  ...
  ap.add_argument(
      ...
      default=0.8,
      help=(
          "Single sampling temperature (used only when --temperatures is not "
          "given)."
      ),
  )
  ap.add_argument(
      ...
      " Each writes its own JSON tagged with the temp:"
      " '<out_stem>_t<temp>.json'. "
  ...
  ap.add_argument(
      "--seed", type=int, default=0, help="SGLang engine random seed."
  )
  ap.add_argument(
      "--max_assistant_turns",
      type=int,
      default=4,
      help="Total solver attempts (1 = single-turn).",
  )
  ...
  ap.add_argument(
      ...
      type=int,
      default=8192,
      help="Cumulative response-token budget (assistant + feedback) before"
      " overflow stop. Default 8192 matches MAX_RESPONSE_LENGTH in the"
      " training launchers.",
  )
  ap.add_argument(
      "--max_prompt_length",
  ...
  ap.add_argument(
      ...
      "'model_feedback' to eval a model-feedback-trained checkpoint "
      "apples-to-apples.",
  )
  ...
  ap.add_argument(
      ...
      type=int,
      default=0,
      help="Combined char budget for the failing-case fields. oracle mode:"
      " budget for the injected feedback turn. model mode: budget inside the"
      " user-model prompt. 0 => derive from --max_prompt_length (same rule as"
      " the agent loop).",
  )
  ap.add_argument(
      "--max_feedback_tokens",
      type=int,
      default=2048,
      help="model mode only: hard cap on the user-model diagnosis length"
      " (tokens). Mirrors +codecontest.max_feedback_tokens. Shares"
      " --max_response_length "
  ...
  ap.add_argument(
      ...
      type=int,
      default=int(os.getenv("ROLLOUT_TP", "1")),
  )
  ap.add_argument(
  ...
  ap.add_argument(
      ...
      help=(
          "Threads used to grade a turn's batch in parallel (env.step is "
          "blocking)."
      ),
  )
  args = ap.parse_args()

  # Derive the feedback char budget the same way CodeRefineAgentLoop does, so
  # the injected feedback text is identical to what the model saw in training.
  if args.max_feedback_chars <= 0:
    args.max_feedback_chars = int(args.max_prompt_length * 3.0 * 0.5)

  if (
      not os.environ.get("CODECONTEST_EXEC_URL")
  ...
  # Each temperature gets its own tagged JSON, e.g. eval_step120.json ->
  # eval_step120_t0.8.json.
  results_index = []
  for temperature in temps:
    # Greedy decoding produces identical samples, so pass@k is degenerate --
    # force 1 sample at temperature 0 (no point spending compute on duplicate
    # trajectories).
    ...
    results_index.append(
        {
            ...
            f"pass@{n_samples}": summary[f"pass@{n_samples}"],
        }
    )
