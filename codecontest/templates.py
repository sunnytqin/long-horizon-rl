...
oracle environment runs it against ground-truth tests and, on failure, feeds
...
SOLVER_SYSTEM_PROMPT = (
    ...
    "standard input and writes to standard output. Think step by step, then "
    "put your final solution in a single ```python ... ``` code block. The "
    "program must be self-contained and runnable as-is."
)
...
standard output (stdout). End your response with the complete solution in a \
single
```python ... ``` code block.
...
# Wraps the failing-case block shown to the solver after an incorrect
# submission. Reflection-style: ask the solver to diagnose the previous attempt
# before rewriting, which tends to produce better refinements than a bare "fix
# it" instruction.
SOLVER_ORACLE_REFLECTION_FEEDBACK_TEMPLATE = (
    ...
    " correct.\n\n{feedback_block}Before writing an improved solution, please"
    ...
    "Please make sure you write a solution based on the reflection you made"
    " and put the new complete code in a python block.\n"
)
...
# diagnosis that the solver used to be asked to write itself. The diagnosis is
# then injected as the next user turn. Ported from the tunix
# FEEDBACK_MODEL_PROMPT_TEMPLATE but as structured chat content (system + user)
# since VERL applies a chat template.
FEEDBACK_MODEL_SYSTEM_PROMPT = (
    "You are a helpful assistant that analyzes code and test failures to"
    " provide diagnostic feedback. Be concise. Do NOT write any code."
)

FEEDBACK_MODEL_USER_TEMPLATE = (
    "A coding problem was given and a solution was attempted, but it failed"
    " some test cases.\n\nProblem:\n{problem}\n\nAttempted solution:\n"
    "```python\n{code}\n"
...
# Wraps the user-model diagnosis as the user turn shown to the SOLVER. The
# concrete failing cases are deliberately NOT included here (the diagnosis
# stands in for them); the solver still sees its own previous code as earlier
# assistant turns.
SOLVER_MODEL_FEEDBACK_TEMPLATE = (
    ...
    "solution. Put the new complete code in a single ```python ... ``` code"
    " block.\n"
)
...
def build_feedback_model_messages(
  ...
  Args:
    ...
    max_total_chars: combined char budget for the failing-case fields,
      applied via ``format_oracle_feedback`` so the feedback prompt stays
      under prompt_length.
...
# Strips the reasoning block a (future) reasoning model might emit before its
# diagnosis. For a plain Instruct model this is a defensive no-op, but keeping
# it here means the training loop and the offline validator normalize diagnoses
# identically.
_THINK_BLOCK = re.compile(r"<think>.*?</think>", re.DOTALL)

# Injected verbatim when the user model returns an empty diagnosis (after the
# <think> strip). Lives here -- not inline in the agent loop -- so both
# pipelines agree on it.
...
def build_model_feedback_user_message(analysis: str) -> str:
  ...
  The diagnosis length is bounded upstream by the user model's
  ``max_new_tokens`` cap (see ``max_feedback_tokens`` in the agent loop), so the
...
def _clip(s: str, cap: Optional[int]) -> str:
  """Clips ``s`` to ``cap`` chars, head+tail, with an honest elision marker.
...
def format_oracle_feedback(
  ...
  """Formats failing cases as feedback text.

  Ported from ``code_util.format_oracle_feedback``.
  ...
  Args:
    ...
    max_total_chars: optional combined budget (chars) across ALL fields of ALL
      ...
      computed over every field and applied uniformly, so the truncation
      lands on
  ...
  """
  # Strip first so the budget reflects exactly what we emit; cap is then
  # computed jointly over all three fields of all cases (the "combined" budget).
  cases = [
