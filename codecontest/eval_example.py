...
def start_workers(
    pretrained_model, gpu_configs, max_model_len, max_generation_token
):
  task_queues = []
  result_queues = []
  processes = []
...
def generate_results_api(prompts):
  ...
  for batch_start in range(0, total, rpm_limit):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
      fut_to_idx = {
          pool.submit(fetch_completion, prompts[i]): i for i in batch_slice
      }
...
def save_prompts_to_jsonl(
    prompts, filename, system_content, model, max_tokens, url
):
...
def extract_completions(raw):
  ...
  assistant_texts = [
      body["choices"][0]["message"]["content"] for body in bodies
  ]
...
def parse_args():
  ...
  parser.add_argument(
      "--pretrained_model", type=str, default=evaluation_config.pretrained_model
  )
  parser.add_argument("--dataset", type=str, default=evaluation_config.dataset)
  parser.add_argument(
      "--use_api", type=str2bool, default=evaluation_config.use_api
  )
  ...
  parser.add_argument(
      "--max_model_len", type=int, default=evaluation_config.max_model_len
  )
  parser.add_argument(
      "--max_generation_token",
      type=int,
      default=evaluation_config.max_generation_token,
  )
  parser.add_argument("--temp", type=float, default=evaluation_config.temp)
  parser.add_argument(
      "--num_chunks", type=int, default=evaluation_config.num_chunks
  )
  parser.add_argument(
      "--no_example", type=str2bool, default=evaluation_config.no_example
  )
  parser.add_argument(
      "--max_test", type=int, default=evaluation_config.max_test
  )
  parser.add_argument(
      "--trust_remote_code",
      type=str2bool,
      default=evaluation_config.trust_remote_code,
  )
  parser.add_argument(
      "--exe_verbose", type=str2bool, default=evaluation_config.exe_verbose
  )
  parser.add_argument(
      "--is_final_eval", type=str2bool, default=evaluation_config.is_final_eval
  )
  parser.add_argument(
      "--single_eval", type=str2bool, default=evaluation_config.single_eval
  )
  parser.add_argument(
      "--scale_tuple_list",
      type=ast.literal_eval,
      default=evaluation_config.scale_tuple_list,
  )
  parser.add_argument(
      "--api_model_name", type=str, default=evaluation_config.api_model_name
  )
  parser.add_argument("--api_key", type=str, default=evaluation_config.api_key)
  parser.add_argument(
      "--base_url", type=str, default=evaluation_config.base_url
  )
  parser.add_argument(
      "--api_temperature", type=float, default=evaluation_config.api_temperature
  )
  parser.add_argument(
      "--max_workers", type=int, default=evaluation_config.max_workers
  )
  parser.add_argument(
      "--use_openai_batch_api",
      type=str2bool,
      default=evaluation_config.use_openai_batch_api,
  )
  parser.add_argument(
      "--max_tokens", type=int, default=evaluation_config.max_tokens
  )
  parser.add_argument(
      "--rpm_limit", type=int, default=evaluation_config.rpm_limit
  )
  parser.add_argument(
      "--gpu_groups",
      type=ast.literal_eval,
      default=evaluation_config.gpu_groups,
  )
  parser.add_argument(
      "--system_prompts", type=str, default=evaluation_config.system_prompts
  )
  parser.add_argument(
      "--system_case_prompts",
      type=str,
      default=evaluation_config.system_case_prompts,
  )
  parser.add_argument(
      "--special_requirements",
      type=str,
      default=evaluation_config.special_requirements,
  )
...
if use_api == False:
  task_queues, result_queues, processes = start_workers(
      pretrained_model, gpu_groups, max_model_len, max_generation_token
  )
  ...
  if use_openai_batch_api:
    api_batch_filename = (
        api_model_name.replace("/", ".") + "-" + dataset + ".jsonl"
    )
...
def get_scaling_prompt(data_i, method):
  ...
  if method == "case":
    ...
    example_output = ", ".join(
        [repr(item) for item in data_i["example_output"]]
    )
    if n_example == 0:
      example_intro = """ """
    if n_example == 1:
...
def extract_test_cases(full_output):
  ...
  if matches_input:
    ...
    matches_input_plain = re.findall(
        pattern_input_plain, full_output, re.DOTALL
    )
...
# generate
if use_api == False:
  shuffled_outputs = generate_results(
      shuffled_prompts, gpu_groups, task_queues, result_queues
  )
else:
  if use_openai_batch_api:
...
for full_output in code_generation_result:
  ...
  data[index_i]["full_code_generation"] = data[index_i][
      "full_code_generation"
  ] + [full_output]
  data[index_i]["generated_code"] = data[index_i]["generated_code"] + [
      code_output
  ]
  i += 1
...
for full_output in case_generation_result:
  ...
  data[index_i]["full_case_generation"] = data[index_i][
...
# output the data
os.makedirs(
    os.path.dirname("./temp_data/outputs-" + outputs_name + ".json"),
    exist_ok=True,
)
with open(
    "./temp_data/outputs-" + outputs_name + ".json", "w", encoding="utf-8"
) as f:
...
def execute_scripts(outputs_name, num_chunks):
  ...
  test_exe_results = run_scripts_with_chunk(
      test_code_list, test_input_list, test_time_limit_list, worker, num_chunks
  )
  cprint("execution job done!", "green")
  ...
  for i in range(len(data)):
    ...
    if (
        data[i]["case_exe_results"] is None
        or data[i]["test_exe_results"] is None
    ):
      continue
  ...
  if is_final_eval:
    if use_api == False:
      outputs_result_name = (
          ...
          + pretrained_model.replace("/", ".")
          + "-final_eval.txt"
      )
    else:
      outputs_result_name = (
          "./results/results-eval-"
          + api_model_name.replace("/", ".")
          + "-final_eval.txt"
      )
  else:
