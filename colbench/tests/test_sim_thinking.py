...
def test_thinking_flag_is_nested_under_chat_template_kwargs(monkeypatch):
    ...
    assert "enable_thinking" not in eb, "top-level enable_thinking is accepted-and-ignored by SGLang"
...
def test_strip_think_truncated_block_is_removed():
    ...
    assert strip_think(truncated) == "", "unterminated reasoning must not reach the dialogue"
