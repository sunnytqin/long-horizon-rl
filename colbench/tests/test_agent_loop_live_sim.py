"""Agent-loop tests for the GT path's LIVE-weights simulator.

Selected by ``+colbench.sim_live=True``.

The arm under test keeps ONE copy of the weights for the whole run: the user
turn is generated by ``server_manager.generate`` (the training rollout engine)
instead of an HTTP call to a frozen sim server. What must hold:

  * the sim's prompt is BYTE-IDENTICAL to the frozen path's (same
    env._build_sim_prompt), so the two arms differ only in which weights answer
    -- and it still carries the hidden GT;
  * the sim's generated TOKENS never enter the solver trajectory (only the
    <=400-char text does, at mask=0), so the leak invariant and the mask
...
colbench/tests/test_agent_loop_live_sim.py -v
"""
...
def _make_loop(
  ...
  """Construct a ColBenchAgentLoop bypassing AgentLoopBase.__init__.

  Wired to one fake engine.
  """
...
def test_live_sim_prompt_carries_hidden_gt_and_never_leaks_to_solver():
  obj = _make_loop(
      solver_turns=["What's the cutoff?", _answer_turn(GT)],
      sim_replies=["It's 10."],
  )
...
def test_frozen_path_untouched_when_sim_live_false():
  ...
  # env.sim_backend (patched here to a stub, standing in for the frozen HTTP
  # server).
