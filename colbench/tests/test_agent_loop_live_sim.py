...
class _RoleAwareServerManager:
  ...
  def __init__(self, tokenizer, solver_turns, sim_replies):
    ...
    self.sim_prompts = (
        []
    )  # decoded sim prompts (assert GT presence / prompt identity)
...
def test_live_sim_generates_on_the_training_engine():
  ...
  assert (
      sm.sim_ids[0] not in sm.solver_ids
...
def test_live_sim_rejection_sampling_resamples_until_clean():
  ...
  obj = _make_loop(
      ...
      sim_replies=[
          ...
          "I'd rather describe it: use a cutoff of 10.",
      ],
  ...
  assert (
      len(set(obj.server_manager.sim_ids)) == 2
  )  # a fresh request_id per sample
...
def test_sim_timeout_is_observable_and_kills_the_episode_at_reward_zero():
  ...
  async def slow_sim(*, request_id, prompt_ids, sampling_params, **kwargs):
    if (
        obj.server_manager.solver_ids
        and request_id != obj.server_manager.solver_ids[0]
    ):
  ...
  assert (
      rei["answered"] == 0.0 and out.reward_score == 0.0
  )  # the artifact GRPO would train on
...
def test_timeout_keys_present_on_the_happy_path():
  ...
  assert (
      rei["sim_seconds"] > 0.0 and rei["sim_seconds_max"] >= rei["sim_seconds"]
  )
...
def test_frozen_path_untouched_when_sim_live_false():
  ...
  obj = _make_loop(
      solver_turns=["Q?", _answer_turn(GT)], sim_replies=[], sim_live=False
  )
