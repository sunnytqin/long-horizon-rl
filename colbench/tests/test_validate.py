...
class FakeLLM:
  ...
  def generate(self, prompt, sampling_params):
    ...
    self.turn += 1
    return [
        {"text": text, "meta_info": {"completion_tokens": len(text.split())}}
        for _ in prompt
    ]
...
def test_rejection_on_clean_backend_records_no_retry(tmp_path):
  ...
  assert (
      rj["n_user_turns_accepted"] == 4
  )  # 2 problems x 2 samples, one sim turn each
...
def test_rejection_exhaustion_marks_simulation_failure(tmp_path):
  ...
  assert (
      rj["reject_reason_counts"].get("def", 0) == 4 * 4
  )  # 4 tries x 4 trajectories
  for traj in dump["trajectories"]:
