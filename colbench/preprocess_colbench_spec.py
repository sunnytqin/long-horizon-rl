...
def _usable(spec_row: dict[str, Any]) -> bool:
  ...
  if not spec_row.get("ok", True):
    return False
  return bool(str(spec_row.get("requirements", "")).strip()) and bool(
      str(spec_row.get("plot", "")).strip()
  )
    ...
    rows.append(
        {
            ...
            "prompt": [
                {
                    "role": "system",
                    "content": COLBENCH_SPEC_AGENT_SYSTEM_PROMPT,
                },
                {
                    "role": "user",
...
def main():
  ...
  ap.add_argument(
      "--split",
      default="train",
      help="Value stored in extra_info.split (metadata).",
  )
  args = ap.parse_args()

  rows = build_rows(
      os.path.expanduser(args.raw_parquet),
      os.path.expanduser(args.specs_jsonl),
      args.split,
  )
  if not rows:
    raise SystemExit(
