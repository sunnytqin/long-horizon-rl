...
class Trajectory:
  ...
  def __init__(
    ...
    ]  # solver view: [system, user(problem), ...]
    self.sim_dialogue = [
    ...
    self.last_code = ""
    self.first_code = (
        ""  # the assistant's FIRST code proposal (for the lift diagnostic)
    )
  ...
  def to_dict(self):
    return {
        ...
        "code_proposals": self.code_proposals,
        ...
        "first_code_pass_rate": self.first_code_reward,
        "first_code_all_pass": self.first_code_all_pass,
        "first_code": self.first_code,
        "final_code": self.last_code,
  ...
  def generate(self, message_lists, tokenizer, max_model_len):
    ...
    for msgs in message_lists:
      ptext = tokenizer.apply_chat_template(
          msgs, add_generation_prompt=True, tokenize=False, **self._tk
      )
      room = (
          max_model_len
          - len(tokenizer.encode(ptext, add_special_tokens=False))
          - CTX_MARGIN
      )
      sp = dict(self._sp)
    ...
    return [
        {
            "text": o["text"],
            "tokens": int(o["meta_info"].get("completion_tokens", 0)),
        }
        for o in outs
    ]
  ...
  for row_index, row in val_df.iterrows():
    ...
    for s in range(n_samples):
      ...
      trajs.append(
          Trajectory(
              int(row_index),
              task_id if task_id is not None else int(row_index),
              s,
              messages,
              problem_text,
              env,
  ...
  for turn in range(args.max_assistant_turns):
    ...
    for t in active:
      if t.response_tokens >= t.response_budget:
        t.overflow = True
        t.done = True
        t.terminated_by = t.terminated_by or (
    ...
    if not gen_batch:
      ...
      pending = []
      for t, out in zip(gen_batch, outs, strict=False):
        text = out["text"]
...
def _aggregate(trajs, temperature, n_samples, args, elapsed):
  ...
  mean_first_pass_showed = (
      (sum(t.first_code_reward for t in showed) / len(showed))
      if showed
      else None
  )
  mean_final_pass_showed = (
      (sum(t.reward for t in showed) / len(showed)) if showed else None
  )
  ...
  mean_best = sum(
      max(t.reward for t in grp) for grp in by_problem.values()
  ) / max(1, len(by_problem))
  pass_at_1 = sum(grp[0].reward for grp in by_problem.values()) / max(
      1, len(by_problem)
  )
  ...
  return {
      ...
      "mean_first_code_pass_rate_showed": (
          round(mean_first_pass_showed, 4)
          if mean_first_pass_showed is not None
          else None
      ),
      "mean_final_code_pass_rate_showed": (
          round(mean_final_pass_showed, 4)
          if mean_final_pass_showed is not None
          else None
      ),
      "mean_code_proposals": round(mean_code_proposals, 3),
      "mean_assistant_turns": round(mean_turns, 3),
...
def main():
  ...
  ap.add_argument(
      ...
      default=os.path.expanduser(
          "~/data/colbench_spec/selfplay_cond30.parquet"
      ),
  ...
  ap.add_argument(
      "--solver_concurrency",
      type=int,
      default=int(os.getenv("SOLVER_CONCURRENCY", "16")),
  ...
  ap.add_argument(
      ...
      default=os.environ.get(
          "SIM_OPENAI_BASE_URL", "https://api.openai.com/v1"
      ),
  ...
  ap.add_argument(
      "--sim_temperature",
      type=float,
      default=float(os.environ.get("SIM_TEMPERATURE", "1.0")),
  )
  ap.add_argument(
      "--sim_top_p",
      type=float,
      default=float(os.environ.get("SIM_TOP_P", "1.0")),
  )
  ap.add_argument(
      "--sim_max_tries",
      type=int,
