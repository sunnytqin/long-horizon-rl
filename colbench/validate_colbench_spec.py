...
class OpenAISolver:
    ...
    def __init__(self, base_url, model, temperature, top_p, top_k, max_new_tokens, enable_thinking, concurrency):
...
class SGLangSolver:
    ...
    def __init__(self, engine, template_kwargs, temperature, top_p, top_k, max_new_tokens):
        ...
        self._sp = {"temperature": temperature, "top_p": top_p, "top_k": top_k, "max_new_tokens": max_new_tokens}
...
def run_eval(solver, tokenizer, val_df, temperature, n_samples, args, out_path, max_model_len, sim_backend=None):
        ...
        if not gen_batch:
            ...
            for t, reply, raw, code_rejected, exhausted in pool.map(_sim, pending):
                ...
                if templates.sim_terminated(raw):
                    ...
                    continue
                feedback_tokens = (
                    len(tokenizer.encode(reply, add_special_tokens=False)) if tokenizer else len(reply) // 4
                )
    ...
    with open(out_path, "w") as f:
        json.dump(
            {"summary": summary, "num_saved_conversations": len(saved), "trajectories": [t.to_dict() for t in saved]},
            f,
            indent=2,
        )
...
def _aggregate(trajs, temperature, n_samples, args, elapsed):
    ...
    return {
        ...
        "mean_first_code_pass_rate_showed": (
            round(mean_first_pass_showed, 4) if mean_first_pass_showed is not None else None
        ),
        "mean_final_code_pass_rate_showed": (
            round(mean_final_pass_showed, 4) if mean_final_pass_showed is not None else None
        ),
...
def build_solver(args, tokenizer, max_model_len):
    ...
    return SGLangSolver(
        engine, _solver_template_kwargs(), args.temperature, args.top_p, args.top_k, args.max_new_tokens_per_turn
    )
...
def main():
    ...
    ap.add_argument(
        "--val_file",
        default=os.path.expanduser("~/data/colbench_spec/selfplay_cond30.parquet"),
    ...
    ap.add_argument(
        "--sim_base_url",
        default=os.environ.get("SIM_OPENAI_BASE_URL", "https://api.openai.com/v1"),
    ...
    for temperature in temps:
        ...
        summary = run_eval(
            solver, tokenizer, val_df, temperature, n_samples, args, out_path, max_model_len, sim_backend=sim_backend
        )
