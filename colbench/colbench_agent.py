...
Structurally a sibling of
``codecontest.model_feedback_agent.ModelFeedbackAgentLoop`` (same
budget/overflow handling, weights-version bookkeeping, response-mask
construction, ``AgentLoopOutput`` assembly, and
``masking.apply_train_turns_mask`` for the SET-2 study). The differences:

  * The between-turns user turn is produced EVERY non-terminal turn (not only on
    failure) by ``env.generate_user_turn``. Two arms, selected by
    ``+colbench.sim_live``:
      - default (False) = FROZEN sim: an HTTP call to a separate SGLang server
        holding the base weights, which never update while the solver trains
        away from them.
      - ``sim_live=True`` = LIVE sim: the user turn is generated by
        ``server_manager.generate`` on the TRAINING rollout engine, so the
        simulator IS the current policy -- one set of weights for both roles, no
        frozen copy, no second server. Either way the reply is injected with
        mask=0 (no gradient) and the hidden GT never enters the solver
        trajectory.
  * There is no mid-turn grading: the solver interacts to extract requirements
    and submits once (``I WANT TO ANSWER:``); the code is graded ONCE at the end
    and the trajectory reward is the fractional GT pass-rate.
  * Optional user-sim rejection sampling (``+colbench.sim_reject_max_tries``,
    default 0=off) resamples the sim turn until it leaks no code, closing the
    exploit where the solver just asks the simulator to write the function. If
    every sample leaks, the solver's turn was one the sim can only answer with
    the solution: the conversation ends there and the trajectory keeps reward 0
    (identical to running out of turns without submitting) but STAYS in the
...
NOT a flag flip here: the sim's tokens are sampled from a DIFFERENT prompt (one
that contains the hidden GT), so they cannot be relabelled mask=1 inside the
solver's sequence -- it needs a second trajectory per episode, and a reward for
the user role that is not "the solver succeeded" (that objective pays the sim to
leak the GT).
"""
...
class ColBenchAgentLoop(AgentLoopBase):
  ...
  def _make_live_sim_backend(self):
    ...
    answers*:
      ...
      * FRESH ``request_id`` per sim call. The solver's turns deliberately share
        ...
        sequence (it contains the hidden GT), so it must not be appended to that
        session.
      * The sim's generated TOKENS are dropped after decoding. Only the
        <=400-char TEXT re-enters the solver's context with mask=0, exactly as
        on the frozen-server path, so the leak invariant and the trajectory's
  ...
  async def run(
    ...
    # give an np.ndarray. Convert via an explicit None check to stay safe under
    # both (`x or []` would call bool() on an ndarray and raise).
    ...
    sim_leaks = 0
    # Sim LATENCY + timeout, surfaced via reward_extra_info because
    # AgentLoopOutput.metrics is a fixed-field pydantic model
    # (generate_sequences/tool_calls/compute_score/num_preempted) with
    # extra='ignore' -- anything else put in `metrics` is silently DROPPED at
    # validation and never reaches TensorBoard. Under sim_live the sim shares
    # the rollout engine's queue, so `sim_seconds` is the leading indicator and
    # `sim_turn_timeout` the damage: a timeout BREAKS the episode, and an
    # unanswered episode then trains at reward 0 -- an infrastructure artifact
    # that GRPO cannot tell from a genuine failure, biased toward LONG dialogues
    # (more sim calls = more chances to trip). Watch it before trusting any
    ...
    for turn in range(self.max_assistant_turns):
      ...
      if not res["accepted"]:
        ...
        #            the simulator can only answer WITH the solution ("just give
        #            me the final function"). End the conversation here. reward
        #            stays 0.0 -- the same outcome as running out of turns
        #            without submitting -- and the trajectory STAYS in the
        #            batch, so its solver turns take the group-relative negative
        #            advantage. That gradient is the point: it is what makes
        #            this differ from discarding.
        sim_failed = True
    ...
    output = AgentLoopOutput(
        ...
        extra_fields={
            ...
            # MUST live here. Keys must be identical across all rollouts (verl
            # reads the key set from the first sample).
            "reward_extra_info": {
