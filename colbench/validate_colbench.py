...
class Trajectory:
  ...
  def __init__(
    ...
    self.response_budget = (
        response_budget  # max cumulative response tokens (solver + sim replies)
    )
...
def eval_tagged_path(
  ...
  reject_tag = (
      f"_reject{sim_reject_max_tries}"
...
def run_eval(
  ...
  for turn in range(args.max_assistant_turns):
    ...
    pending = (
        []
    )  # trajectories that did not answer and can still get a sim reply
    for t, out in zip(kept, outputs, strict=False):
      ...
      t.turn_records.append(
          {"turn": turn, "answered": has_answer, "is_last": is_last_turn}
      )
      if has_answer:
        t.answered = True
  ...
  mean_pass_rate = sum(t.reward for t in valid) / max(
      1, n_valid
  )  # primary (val-core analog)
  all_pass_rate = sum(1 for t in valid if t.all_pass) / max(
      1, n_valid
  )  # fully correct
  ...
  pass_at_1 = sum(grp[0].reward for grp in valid_by_problem.values()) / max(
  ...
  rejection = {
      ...
      "mean_samples_per_turn": round(
          total_samples / max(1, len(all_events)), 3
      ),
...
def main():
  ...
  ap.add_argument(
      "--out",
      required=True,
      help="Output JSON path stem for the conversation dump.",
  )
  ap.add_argument(
      "--max_problems",
      type=int,
  ...
  ap.add_argument(
      ...
      type=int,
      default=1,
      help="Trajectories sampled per problem.",
  )
  ap.add_argument(
      "--max_saved_convos",
      type=int,
  ...
  ap.add_argument(
      ...
      type=int,
      default=0,
      help="SGLang engine + saved-sample RNG seed.",
  )
  ap.add_argument(
      "--max_assistant_turns",
      type=int,
      default=10,
      help="Total solver turns (clarify + submit).",
  )
