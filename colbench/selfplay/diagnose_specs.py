...
def _aggregate(rows: list[dict[str, Any]]) -> dict[str, Any]:
  ...
  pass_at_n = sum(
      any(s["all_pass"] for s in samples) for samples in by_task.values()
  ) / len(by_task)
  return {
      "n_tasks": len(by_task),
...
def diagnose_specs_file(
  ...
  backend = (
      specs[0].get("backend")
      if specs and specs[0].get("backend")
      else os.path.basename(specs_path)
  )
  label = backend
  jobs, n_missing = [], 0
  for spec in specs:
  ...
  with ThreadPoolExecutor(max_workers=max(1, concurrency)) as pool:
    futs = [
        pool.submit(_solve_and_grade, endpoint, task, spec, reward_time_limit)
...
def main():
  ...
  ap.add_argument(
      ...
      type=int,
      default=None,
      help="Limit #tasks loaded (must cover the specs' indices).",
  )
  ap.add_argument(
      "--specs",
      nargs="+",
      required=True,
      help="One or more spec JSONL files to diagnose.",
  )
  ap.add_argument(
      "--out",
      default=None,
      help="Optional JSON path to dump per-file metrics + rows.",
  )
  # Solver endpoint.
  ap.add_argument(
      "--solver_base_url",
      default=os.environ.get("SOLVER_BASE_URL", "http://127.0.0.1:30000/v1"),
  )
  ap.add_argument("--solver_model", default=os.environ.get("SOLVER_MODEL", ""))
  ap.add_argument(
      "--solver_api_key", default=os.environ.get("SOLVER_API_KEY", "EMPTY")
  )
  ...
  ap.add_argument(
      ...
      type=int,
      default=1,
      help="Solver samples per spec (pass@n).",
  )
  ap.add_argument(
      "--reward_time_limit",
      type=float,
      default=6.0,
      help="Per-case GT exec timeout (s).",
  )
  ap.add_argument(
      "--concurrency",
      type=int,
