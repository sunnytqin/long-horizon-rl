...
def _author_one(endpoint: ChatEndpoint, task: dict, backend_label: str, mode: str = "static") -> dict:
    ...
    if mode == "plot":
        ...
        raw = endpoint.chat(spec_templates.build_author_messages(task["problem_description"], task["ground_truth"]))
...
def main():
    ...
    ap.add_argument(
        "--out", default=None, help="Output JSONL (default: <data_dir>/specs/<stem>.<backend>[.plot].jsonl)."
    )
    # Author endpoint.
    ...
    with ThreadPoolExecutor(max_workers=max(1, args.concurrency)) as pool:
        ...
        for fut in as_completed(futs):
            ...
            if n_done % args.flush_every == 0:
                print(f"[generate_specs] {n_done}/{len(todo)} authored ({n_ok} parsed ok) in {time.time() - t0:.0f}s")
    if buf:
        append_jsonl(out, buf)
