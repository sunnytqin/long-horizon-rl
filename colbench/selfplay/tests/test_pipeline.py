...
def test_openai_vendor_drops_vendor_extras_and_uses_completion_tokens():
  ep = ChatEndpoint(
      base_url="https://api.openai.com/v1", model="gpt-x", vendor="openai"
  )
  assert ep._extra_body() == {}  # no top_k/min_p to the public API
  psets = ep._param_sets([{"role": "user", "content": "hi"}])
  assert all(
      "max_completion_tokens" in p and "max_tokens" not in p for p in psets
  )
  assert "extra_body" not in psets[0]
  # progressively-minimal: full sampling -> temperature-only -> bare
  assert (
      "top_p" in psets[0]
      and "top_p" not in psets[1]
      and "temperature" not in psets[2]
  )


def test_vllm_vendor_sends_extra_body():
  ep = ChatEndpoint(
      base_url="http://127.0.0.1:30000/v1", model="specgen"
  )  # default vendor=vllm
...
def test_resolve_gt_preprocessed_schema():
  ...
  assert (
      t["ground_truth"] == GT
      and t["test_cases"] == CALLS
      and t["problem_description"] == PROBLEM
  )
...
def test_author_one_builds_record():
  ...
  assert (
      rec["index"] == 3 and rec["backend"] == "selfplay" and rec["ok"] is True
  )
...
def test_generate_is_resumable():
  with tempfile.TemporaryDirectory() as d:
    ...
    append_jsonl(
        p,
        [
            {"index": 0, "backend": "selfplay"},
            {"index": 2, "backend": "selfplay"},
        ],
    )
...
def test_author_one_plot_builds_record():
  ...
  rec = generate_specs._author_one(
      _endpoint(PLOT_SPEC_JSON), task, "selfplay", mode="plot"
  )
...
def test_diagnose_plot_specs_grade_on_requirements():
  ...
  assert r["label"] == "selfplay"
  assert (
      r["metrics"]["solve_rate"] == 1.0
  )  # graded off requirements, plot ignored
...
def test_diagnose_skips_specs_missing_requirements():
  ...
  with tempfile.TemporaryDirectory() as d:
    ...
    append_jsonl(
        p, [{"index": 0, "backend": "selfplay", "mode": "plot", "plot": "x"}]
    )  # no requirements
    r = diagnose_specs.diagnose_specs_file(
        p,
        tasks_by_index,
