...
def test_author_messages_contain_both_public_and_gt():
  ...
  assert (
      "Describe intent" in user
      or "never code" in user
      or "not code" in user.lower()
  )
...
def test_plot_solver_still_uses_requirements():
  ...
  assert (
      "forgets an edge case" not in user
  )  # plot is not fed to the diagnostic solver
